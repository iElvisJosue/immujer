...
import useBuscarUsuariosPorFiltro from "../../../hooks/usuarios/Lista/useBuscarUsuariosPorFiltro";
// COMPONENTES A USAR
...
// ESTILOS A USAR
import "../../../styles/components/usuarios/Lista/ListaCompletaUsuarios.css";

export default function ListaCompletaUsuarios({
  PropsVista: { establecerSubvistaActual },
  PropsUsuario: { idUsuario, establecerDetallesUsuario },
  ...
  const EstablecerUsuarioSeleccionado = (infUsuario) => {
    if (infUsuario.rol === "Admin") {
      ...
      establecerDetallesUsuario(infUsuario);
      ...
      {cargandoUsuarios ? (
        ...
        listaDeUsuarios.map((infUsuario) => (
          <InformacionRegistro
            ...
            Titulo={`${infUsuario.nombre} ${infUsuario.apellido_paterno} ${
              infUsuario.apellido_materno
            } ${infUsuario.id_one_signal ? "🔔" : "🔕"}`}
            InfDestacada={{
              Bg: "Rol",
              Texto: infUsuario.rol,
            }}
